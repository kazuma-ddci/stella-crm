...
-- 0. 既存の契約書ドキュメントを削除
-- ============================================
-- マイグレーション 20260409100000_hojo_vendor_contract_refactor が
-- 旧 *_contract_url カラムを自動的にこのテーブルに移行するため、
-- 非URL文字列・"-"・スペース区切りの不正データが含まれている。
-- それを一旦全削除してからクリーンなURLを再投入する。
DELETE FROM hojo_vendor_contract_documents WHERE vendor_id IN (1, 2, 3, 5, 6, 7, 8);
