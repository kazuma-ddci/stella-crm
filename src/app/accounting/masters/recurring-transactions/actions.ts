...
export async function createRecurringTransaction(
  ...
  try {
    ...
    if (intervalCount < 1 || !Number.isInteger(intervalCount)) {
      return err("繰り返し間隔は1以上の整数で指定してください");
    }

    // 固定金額の場合、金額は必須
    if (amountType === "fixed" && (amount === null || amount === 0)) {
      return err("固定金額の場合、金額は必須です");
    }
    ...
    if (executionDay !== null) {
      if (frequency === "monthly" || frequency === "yearly") {
        if (executionDay < 1 || executionDay > 31) {
          return err("実行日は1〜31の範囲で指定してください");
        }
      }
      if (frequency === "weekly") {
        if (executionDay < 0 || executionDay > 6) {
          return err("曜日は0（日）〜6（土）の範囲で指定してください");
        }
      }
    }
    ...
    if (costCenterId) {
      ...
      if (!cc) return err("指定したプロジェクトが見つかりません");
    }
    ...
    if (allocationTemplateId) {
      ...
      if (!at) return err("指定した按分テンプレートが見つかりません");
    }
    ...
    if (paymentMethodId) {
      ...
      if (!pm) return err("指定した決済手段が見つかりません");
    }
    ...
    if (projectId) {
      ...
      if (!pj) return err("指定したプロジェクトが見つかりません");
    }
    ...
    console.error("[createRecurringTransaction] error:", e);
  ...
  try {
    ...
    if ("counterpartyId" in data) {
      const counterpartyId = data.counterpartyId
        ? Number(data.counterpartyId)
        : null;
      if (!counterpartyId) return err("取引先は必須です");
      const c = await prisma.counterparty.findFirst({
        where: { id: counterpartyId, deletedAt: null },
        select: { id: true },
      });
      if (!c) return err("指定した取引先が見つかりません");
    ...
    if ("expenseCategoryId" in data) {
      const expenseCategoryId = data.expenseCategoryId
        ? Number(data.expenseCategoryId)
        : null;
      if (!expenseCategoryId) return err("費目は必須です");
      const ec = await prisma.expenseCategory.findFirst({
        where: { id: expenseCategoryId, deletedAt: null },
        select: { id: true },
      });
      if (!ec) return err("指定した費目が見つかりません");
    ...
    if ("startDate" in data) {
      if (!data.startDate) return err("開始日は必須です");
    ...
    if ("costCenterId" in data) {
      ...
      if (costCenterId) {
        ...
        if (!cc)
          return err("指定したプロジェクト（按分先）が見つかりません");
      }
    ...
    if ("approverStaffId" in data) {
      ...
      if (approverStaffId) {
        ...
        if (!s) return err("指定した承認者が見つかりません");
      }
    ...
    console.error("[updateRecurringTransaction] error:", e);
    return err(e instanceof Error ? e.message : "予期しないエラーが発生しました");
  }
}
...
export async function deleteRecurringTransaction(
  ...
  try {
    ...
    console.error("[deleteRecurringTransaction] error:", e);
    return err(e instanceof Error ? e.message : "予期しないエラーが発生しました");
  }
}
