...
export async function createAutoJournalRule(data: Record<string, unknown>) {
  ...
  if (!debitAccountId || isNaN(debitAccountId)) {
    throw new Error("借方科目は必須です");
  }
  if (!creditAccountId || isNaN(creditAccountId)) {
    throw new Error("貸方科目は必須です");
  }
  ...
  if (debitAccountId === creditAccountId) {
    throw new Error("借方科目と貸方科目は異なる科目を指定してください");
  }
  ...
  if (!foundIds.has(debitAccountId)) {
    throw new Error("指定された借方科目が見つかりません");
  }
  if (!foundIds.has(creditAccountId)) {
    throw new Error("指定された貸方科目が見つかりません");
  }
  ...
  if (counterpartyId) {
    ...
    if (!cp) {
      throw new Error("指定された取引先が見つかりません");
    }
  }
  ...
  if (expenseCategoryId) {
    ...
    if (!ec) {
      throw new Error("指定された費目が見つかりません");
    }
  }
...
export async function deleteAutoJournalRule(id: number) {
  const session = await getSession();
  const staffId = session.id;
...
export async function reorderAutoJournalRules(orderedIds: number[]) {
  const session = await getSession();
  const staffId = session.id;
...
export async function executeAutoJournal(transactionIds: number[]) {
  const session = await getSession();
  const staffId = session.id;

  if (!transactionIds.length) {
    throw new Error("対象取引が選択されていません");
  }
  ...
  if (!transactions.length) {
    throw new Error("経理処理待ちの取引が見つかりません");
  }
...
export async function suggestRuleFromJournal(journalEntryId: number) {
  ...
  if (!entry.transaction) {
    throw new Error("取引に紐づいていない仕訳からはルール提案できません");
  }
  ...
  if (!debitLine || !creditLine) {
    throw new Error("借方または貸方の明細がありません");
  }
