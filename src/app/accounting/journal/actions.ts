...
// ============================================
// 9. getDueUnrealizedJournalEntries（仕訳日が今日以前の未実現仕訳一覧）
// ============================================

export async function getDueUnrealizedJournalEntries() {
...
export type DueUnrealizedJournalEntry = Awaited<
  ReturnType<typeof getDueUnrealizedJournalEntries>
>[number];

// ============================================
// 10. bulkRealizeJournalEntries（一括実現）
// ============================================

export async function bulkRealizeJournalEntries(ids: number[]): Promise<ActionResult<{ count: number }>> {
  try {
    const staff = await requireStaffForAccounting("edit");
    const uniqueIds = [...new Set(ids.filter((id) => Number.isInteger(id) && id > 0))];

    if (uniqueIds.length === 0) {
      return err("実現にする仕訳を選択してください");
    }
