...
type Props = { data: BbsAccountData[]; staffId: number };

export function BbsTab({ data, staffId }: Props) {
  ...
  const handleDelete = async (id: number) => {
    ...
    await handleAction(id, () => deleteBbsAccount(id));
  };
    ...
    <>
      <div className="border rounded-lg overflow-auto">
        <Table>
          ...
          <TableBody>
            {data.length === 0 ? (
              <TableRow><TableCell colSpan={9} className="text-center text-gray-500 py-8">登録されたBBS社アカウントはありません</TableCell></TableRow>
            ) : data.map((account) => (
              <TableRow key={account.id}>
                ...
                <TableCell>
                  <div className="flex gap-1 flex-wrap">
                    {account.status === "pending_approval" && <Button size="sm" onClick={() => handleAction(account.id, () => approveBbsAccount(account.id, staffId))} disabled={loading === account.id}>認証</Button>}
                    {account.status === "active" && <Button size="sm" variant="outline" onClick={() => handleAction(account.id, () => suspendBbsAccount(account.id))} disabled={loading === account.id}>停止</Button>}
                    {account.status === "suspended" && <Button size="sm" variant="outline" onClick={() => handleAction(account.id, () => reactivateBbsAccount(account.id))} disabled={loading === account.id}>有効化</Button>}
