...
export async function addToolRegistrationStatus(data: Record<string, unknown>): Promise<ActionResult> {
  try {
    ...
    console.error("[addToolRegistrationStatus] error:", e);
    return err(e instanceof Error ? e.message : "予期しないエラーが発生しました");
  }
}

export async function updateToolRegistrationStatus(id: number, data: Record<string, unknown>): Promise<ActionResult> {
  try {
    await requireProjectMasterDataEditPermission();
    ...
    console.error("[updateToolRegistrationStatus] error:", e);
    return err(e instanceof Error ? e.message : "予期しないエラーが発生しました");
  }
}

export async function deleteToolRegistrationStatus(id: number): Promise<ActionResult> {
  try {
    await requireProjectMasterDataEditPermission();
    const usageCount = await prisma.hojoVendor.count({
    ...
    console.error("[deleteToolRegistrationStatus] error:", e);
    return err(e instanceof Error ? e.message : "予期しないエラーが発生しました");
  }
}

export async function reorderToolRegistrationStatuses(orderedIds: number[]): Promise<ActionResult> {
  try {
    await requireProjectMasterDataEditPermission();
    await prisma.$transaction(
      orderedIds.map((id, index) =>
    ...
    console.error("[reorderToolRegistrationStatuses] error:", e);
    return err(e instanceof Error ? e.message : "予期しないエラーが発生しました");
  }
}
