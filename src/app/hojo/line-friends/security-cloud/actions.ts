...
const REVALIDATE_PATH = "/hojo/line-friends/security-cloud";
...
export async function addLineFriend(data: Record<string, unknown>): Promise<ActionResult> {
  ...
  const existing = await prisma.hojoLineFriendSecurityCloud.findUnique({ where: { uid } });
  if (existing) return err(`UID「${uid}」は既に使用されています`);

  await prisma.hojoLineFriendSecurityCloud.create({
    data: {
  ...
  return ok();
    ...
    console.error("[addLineFriend:security-cloud] error:", e);
...
export async function updateLineFriend(id: number, data: Record<string, unknown>): Promise<ActionResult> {
  ...
  if (data.uid !== undefined) {
    ...
    const current = await prisma.hojoLineFriendSecurityCloud.findUnique({ where: { id } });
    if (current && current.uid !== newUid) {
      const existing = await prisma.hojoLineFriendSecurityCloud.findUnique({ where: { uid: newUid } });
  ...
  if (Object.keys(updateData).length > 0) {
    await prisma.hojoLineFriendSecurityCloud.update({ where: { id }, data: updateData });
  }

  revalidatePath(REVALIDATE_PATH);
  return ok();
  } catch (e) {
    console.error("[updateLineFriend:security-cloud] error:", e);
    return err(e instanceof Error ? e.message : "予期しないエラーが発生しました");
  }
}
...
export async function deleteLineFriend(id: number): Promise<ActionResult> {
  try {
    ...
    console.error("[deleteLineFriend:security-cloud] error:", e);
    return err(e instanceof Error ? e.message : "予期しないエラーが発生しました");
  }
}
...
export async function triggerProLineSync(): Promise<{
  ...
  try {
    ...
    const res = await fetch(
      `${triggerUrl}/trigger?secret=${encodeURIComponent(cronSecret)}&account=security-cloud${envQuery}`,
      {
    ...
    console.error("[triggerProLineSync:security-cloud] エラー:", message);
