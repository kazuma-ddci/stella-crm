...
const REVALIDATE_PATH = "/hojo/line-friends/shinsei-support";
...
export async function addLineFriend(data: Record<string, unknown>): Promise<ActionResult> {
  ...
  const existing = await prisma.hojoLineFriendShinseiSupport.findUnique({ where: { uid } });
  if (existing) return err(`UID「${uid}」は既に使用されています`);

  await prisma.hojoLineFriendShinseiSupport.create({
    data: {
  ...
  return ok();
    ...
    console.error("[addLineFriend:shinsei-support] error:", e);
...
export async function updateLineFriend(id: number, data: Record<string, unknown>): Promise<ActionResult> {
  ...
  if (data.uid !== undefined) {
    ...
    const current = await prisma.hojoLineFriendShinseiSupport.findUnique({ where: { id } });
    if (current && current.uid !== newUid) {
      const existing = await prisma.hojoLineFriendShinseiSupport.findUnique({ where: { uid: newUid } });
  ...
  if (Object.keys(updateData).length > 0) {
    await prisma.hojoLineFriendShinseiSupport.update({ where: { id }, data: updateData });
  }

  revalidatePath(REVALIDATE_PATH);
  return ok();
  } catch (e) {
    console.error("[updateLineFriend:shinsei-support] error:", e);
    return err(e instanceof Error ? e.message : "予期しないエラーが発生しました");
  }
}
...
export async function deleteLineFriend(id: number): Promise<ActionResult> {
  try {
    await prisma.hojoLineFriendShinseiSupport.update({
      where: { id },
    ...
    console.error("[deleteLineFriend:shinsei-support] error:", e);
    return err(e instanceof Error ? e.message : "予期しないエラーが発生しました");
  }
}
...
export async function triggerProLineSync(): Promise<{
  ...
  try {
    ...
    const res = await fetch(
      `${triggerUrl}/trigger?secret=${encodeURIComponent(cronSecret)}&account=shinsei-support${envQuery}`,
      {
    ...
    console.error("[triggerProLineSync:shinsei-support] エラー:", message);
