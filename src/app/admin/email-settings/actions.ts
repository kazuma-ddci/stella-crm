  ...
  try {
    ...
    if (data.isDefault) {
      if (!data.smtpHost || !data.smtpUser || !data.smtpPass) {
        return err(
          "デフォルト送信元にするにはSMTP設定（ホスト・ユーザー・パスワード）が必要です"
        );
      }
    }
    ...
    console.error("[addProjectEmail] error:", e);
...
export async function updateEmailSmtp(
  ...
  try {
    ...
    console.error("[updateEmailSmtp] error:", e);
    return err(e instanceof Error ? e.message : "予期しないエラーが発生しました");
  }
}
...
export async function updateProjectEmailMemo(
  ...
  try {
    ...
    console.error("[updateProjectEmailMemo] error:", e);
    return err(e instanceof Error ? e.message : "予期しないエラーが発生しました");
  }
}
...
export async function setProjectEmailDefault(
  ...
  try {
    ...
    if (!pe) return err("設定が見つかりません");

    // SMTP設定チェック
    if (!pe.email.smtpHost || !pe.email.smtpUser || !pe.email.smtpPass) {
      return err(
        "SMTP設定（ホスト・ユーザー・アプリパスワード）が未設定のため、デフォルト送信元に設定できません"
      );
    }
    ...
    console.error("[setProjectEmailDefault] error:", e);
    return err(e instanceof Error ? e.message : "予期しないエラーが発生しました");
  }
}
...
export async function unsetProjectEmailDefault(
  ...
  try {
    ...
    console.error("[unsetProjectEmailDefault] error:", e);
    return err(e instanceof Error ? e.message : "予期しないエラーが発生しました");
  }
}
...
export async function deleteProjectEmail(id: number): Promise<ActionResult> {
  try {
    await requireAdminUser();
    ...
    console.error("[deleteProjectEmail] error:", e);
    return err(e instanceof Error ? e.message : "予期しないエラーが発生しました");
  }
}
