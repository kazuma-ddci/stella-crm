...
export default async function StaffPage() {
  ...
  if (isAdminUser) {
    descriptionItems.push(
      ...
      { label: "各プロジェクト権限", text: "「メンバー」のスタッフに対して設定します。「閲覧」はデータの参照のみ可能です。「編集」にするとデータの作成・更新・外部ユーザー管理・役割の変更が可能になります。「マネージャー」にすると、さらにそのプロジェクトのスタッフ追加・権限管理（編集権限まで付与可）・有効/無効の切り替え・プロジェクト固有の設定変更が可能になります。ファウンダーの場合は自動的に全権限が付与されますが、経費申請の承認者として使うプロジェクトは「承認権限」で個別に設定します" },
