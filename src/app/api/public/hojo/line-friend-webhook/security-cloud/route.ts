...
function verifySecret(request: Request): boolean {
  ...
  const webhookSecret = process.env.HOJO_SECURITY_WEBHOOK_SECRET;
...
export async function GET(request: Request) {
  ...
  try {
    ...
    await prisma.hojoLineFriendSecurityCloud.upsert({
      where: { uid },
    ...
    console.error("[Webhook] hojo/security-cloud failed:", err);
    await logAutomationError({
      source: "webhook/hojo-line-friend/security-cloud",
