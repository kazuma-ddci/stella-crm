...
function verifySecret(request: Request): boolean {
  ...
  const webhookSecret = process.env.HOJO_SHINSEI_WEBHOOK_SECRET;
...
export async function GET(request: Request) {
  ...
  try {
    ...
    await prisma.hojoLineFriendShinseiSupport.upsert({
      where: { uid },
    ...
    console.error("[Webhook] hojo/shinsei-support failed:", err);
    await logAutomationError({
      source: "webhook/hojo-line-friend/shinsei-support",
