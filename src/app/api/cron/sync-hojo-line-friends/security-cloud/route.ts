...
export async function POST(request: Request) {
  ...
  try {
    ...
    const existingRecords = await prisma.hojoLineFriendSecurityCloud.findMany({
    ...
    for (let i = 0; i < friends.length; i++) {
      ...
      if (existing) {
        await prisma.hojoLineFriendSecurityCloud.update({ where: { uid: f.uid }, data });
        updated++;
      } else {
        await prisma.hojoLineFriendSecurityCloud.create({ data: { uid: f.uid, ...data } });
        created++;
      }
    }
    ...
    console.log(`[Cron] sync-hojo-security-cloud: created=${created}, updated=${updated}, total=${total}`);
    return NextResponse.json({ success: true, created, updated, total });
  } catch (err) {
    console.error("[Cron] sync-hojo-security-cloud failed:", err);
    await logAutomationError({
      source: "cron/sync-hojo-line-friends/security-cloud",
