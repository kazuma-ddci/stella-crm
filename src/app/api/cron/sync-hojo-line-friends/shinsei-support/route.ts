...
export async function POST(request: Request) {
  ...
  try {
    ...
    const existingRecords = await prisma.hojoLineFriendShinseiSupport.findMany({
    ...
    for (let i = 0; i < friends.length; i++) {
      ...
      if (existing) {
        await prisma.hojoLineFriendShinseiSupport.update({ where: { uid: f.uid }, data });
        updated++;
      } else {
        await prisma.hojoLineFriendShinseiSupport.create({ data: { uid: f.uid, ...data } });
        created++;
      }
    }
    ...
    console.log(`[Cron] sync-hojo-shinsei-support: created=${created}, updated=${updated}, total=${total}`);
    return NextResponse.json({ success: true, created, updated, total });
  } catch (err) {
    console.error("[Cron] sync-hojo-shinsei-support failed:", err);
    await logAutomationError({
      source: "cron/sync-hojo-line-friends/shinsei-support",
