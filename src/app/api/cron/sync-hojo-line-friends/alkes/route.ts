...
export async function POST(request: Request) {
  ...
  try {
    ...
    const existingRecords = await prisma.hojoLineFriendAlkes.findMany({
    ...
    for (let i = 0; i < friends.length; i++) {
      ...
      if (existing) {
        await prisma.hojoLineFriendAlkes.update({ where: { uid: f.uid }, data });
        updated++;
      } else {
        await prisma.hojoLineFriendAlkes.create({ data: { uid: f.uid, ...data } });
        created++;
      }
    }
    ...
    console.log(`[Cron] sync-hojo-alkes: created=${created}, updated=${updated}, total=${total}`);
    return NextResponse.json({ success: true, created, updated, total });
  } catch (err) {
    console.error("[Cron] sync-hojo-alkes failed:", err);
    await logAutomationError({
      source: "cron/sync-hojo-line-friends/alkes",
