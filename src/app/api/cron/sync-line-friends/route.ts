...
export async function POST(request: Request) {
  ...
  try {
    ...
    // 1. 既存レコードを全件取得
    const existingRecords = await prisma.slpLineFriend.findMany({
    ...
    // 3. 既存でExcelにないレコード（Webhook登録のみ）: 削除しない
    ...
    console.log(
      `[Cron] sync-line-friends: created=${created}, updated=${updated}, total=${total}`
    );

    return NextResponse.json({ success: true, created, updated, total });
  } catch (err) {
    console.error("[Cron] sync-line-friends failed:", err);
    await logAutomationError({
      source: "cron/sync-line-friends",
