...
/**
 * STP 接触履歴 統一版（V2）一覧ページ。
 * URLパラメータで顧客エンティティ絞込み可。
 */
export default async function StpContactHistoriesV2Page({
  searchParams,
}: {
  ...
  const [histories, totalCount, scheduledCount, completedCount] = isFiltered
    ...
    : await Promise.all([
        listContactHistoriesV2({ projectCode: "stp", limit: 200 }),
        countContactHistoriesV2({ projectCode: "stp" }),
        countContactHistoriesV2({ projectCode: "stp", status: "scheduled" }),
        countContactHistoriesV2({ projectCode: "stp", status: "completed" }),
      ]);

  const newHref = isFiltered
    ? `/stp/records/contact-histories-v2/new?targetType=${filterTargetType}${
        filterTargetId !== null ? `&targetId=${filterTargetId}` : ""
      }${filterEntityName ? `&entityName=${encodeURIComponent(filterEntityName)}` : ""}`
    : "/stp/records/contact-histories-v2/new";
        ...
        <div>
          ...
          {isFiltered ? (
            <div className="mt-1 flex items-center gap-2 text-sm text-gray-600">
              ...
              <Link href="/stp/records/contact-histories-v2">
            ...
            <p className="mt-1 text-sm text-gray-600">
              新しい統一接触履歴テーブル。既存STPデータは移行済み。旧ページ（接触履歴）は従来通り動作しています。
            </p>
          )}
        </div>
        <Link href={newHref}>
