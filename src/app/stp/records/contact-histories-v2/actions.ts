...
/**
 * STP 新接触履歴 (V2) の CRUD サーバーアクション。
 * プロジェクトスコープ: stp (edit権限以上)。
...
async function resolveStpProjectId(): Promise<number> {
  ...
  if (!stp) throw new Error("STPプロジェクトが見つかりません");
  return stp.id;
}
...
export async function createContactHistoryV2(
  ...
  try {
    const projectId = await resolveStpProjectId();
    ...
    // Zoom API 発行 (URL 未設定 + hostStaffId 設定済みの meeting)
...
export async function updateContactHistoryV2(
  ...
  try {
    ...
    // Zoom API 発行 (新規追加された URL 未設定 meeting があれば)
    await provisionZoomMeetingsForContactHistory({
      contactHistoryId: id,
      topic: input.title ?? null,
    });
