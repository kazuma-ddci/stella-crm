...
type TargetType = "stp_company" | "stp_agent" | "stp_other";

const TARGET_TYPE_OPTIONS: { value: TargetType; label: string }[] = [
  { value: "stp_company", label: "顧客企業" },
  { value: "stp_agent", label: "代理店" },
  { value: "stp_other", label: "その他" },
];
...
function initialCustomers(initial: ContactHistoryFormInitial | undefined): CustomerFormRow[] {
  ...
  return [{ targetType: "stp_company", targetId: "", attendees: [] }];
}
...
export function ContactHistoryV2Form({
  ...
  const addCustomer = () => {
    setCustomers((prev) => [
      ...prev,
      { targetType: "stp_company", targetId: "", attendees: [] },
    ]);
  };
  ...
  const getTargetIdOptions = (targetType: TargetType) => {
    if (targetType === "stp_company") return masters.companies;
    if (targetType === "stp_agent") return masters.agents;
    return [];
  };

  // STP: stp_company / stp_agent は targetId 必須
  const targetTypeNeedsId = (t: TargetType) =>
    t === "stp_company" || t === "stp_agent";
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      ...
      router.push(`/stp/records/contact-histories-v2/${result.data.id}`);
      router.refresh();
    });
  };
...
function CustomerSection({
  ...
  // STP: stp_company/stp_agent は targetId 選択が必要
  const needsTargetId =
