  ...
  if (histories.length === 0) {
    return (
      <div className="rounded-lg border bg-white p-8 text-center text-sm text-gray-600">
        接触履歴はまだ登録されていません。
        <br />
        データ移行スクリプトを実行すると、既存のSTP接触履歴がここに表示されます。
      </div>
    );
  }
        ...
        <TableBody>
          {histories.map((h) => (
            <TableRow key={h.id} className="cursor-pointer hover:bg-gray-50">
              <TableCell>
                <Link href={`/stp/records/contact-histories-v2/${h.id}`} className="block">
              ...
              <TableCell>
                <Link href={`/stp/records/contact-histories-v2/${h.id}`} className="block hover:underline">
