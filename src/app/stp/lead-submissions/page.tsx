...
export default async function LeadSubmissionsPage() {
  ...
  // 直接顧客用トークン（agentId: null）を取得 or 作成
  let directToken = await prisma.stpLeadFormToken.findFirst({
    where: { agentId: null, status: "active" },
  });
  if (!directToken) {
    directToken = await prisma.stpLeadFormToken.create({
      data: {
  ...
  const directFormToken = directToken.token;
  ...
  const data = submissions.map((s) => ({
    ...
    agentName: s.token.agent?.company.name ?? null,
    status: s.status,
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold">STP リード獲得フォーム回答</h1>
      <Card>
        <CardHeader>
          <CardTitle>フォーム回答一覧</CardTitle>
        </CardHeader>
        <CardContent>
          <SubmissionsTable submissions={data} companyOptions={companyOptions} agentOptions={agentOptions} directFormToken={directFormToken} />
        </CardContent>
      </Card>
    </div>
  );
