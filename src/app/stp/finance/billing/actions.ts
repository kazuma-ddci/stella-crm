...
export type ConfirmBillingItemInput = BillingItemInput & {
...
export async function getBillingLifecycleData(
  ...
  if (contractHistoryIds.length > 0) {
    ...
    for (const item of items) {
      ...
      item.amount = matchingTx.amount;
      item.periodFrom = formatDate(matchingTx.periodFrom);
      item.periodTo = formatDate(matchingTx.periodTo);
      item.description = matchingTx.note ?? matchingTx.expenseCategory?.name ?? item.description;
...
export async function createConfirmedTransactionFromBilling(
  item: ConfirmBillingItemInput
): Promise<ActionResult<{ transactionId: number }>> {
 try {
  ...
  const feeTypeLabel =
    item.feeType === "initial"
      ...
      : item.feeType === "monthly"
        ? "月額"
        : "成果報酬";
  const note = item.note?.trim() || `${contractHistory.company.name} ${feeTypeLabel}`;
  const now = new Date();
  ...
  console.error("[createConfirmedTransactionFromBilling] error:", e);
  return err(e instanceof Error ? e.message : "予期しないエラーが発生しました");
 }
}
...
export async function getExpenseLifecycleData(
  ...
  if (items.length > 0) {
    ...
    for (const item of items) {
      ...
      item.transactionId = matchingTx.id;
      item.amount = matchingTx.amount;
      item.netPaymentAmount = matchingTx.netPaymentAmount;
      item.withholdingTaxAmount = matchingTx.withholdingTaxAmount;
      item.periodFrom = formatDate(matchingTx.periodFrom);
      item.periodTo = formatDate(matchingTx.periodTo);
      item.description = matchingTx.note ?? matchingTx.expenseCategory?.name ?? item.description;
      item.paymentDueDate = matchingTx.paymentDueDate
...
export type ConfirmExpenseItemInput = ExpenseItemInput & {
  expenseCategoryId: number;
  taxAmount: number;
  taxRate: number;
  taxType: "tax_included" | "tax_excluded";
...
export async function createConfirmedTransactionFromExpense(
  input: ConfirmExpenseItemInput
): Promise<ActionResult<{ transactionId: number }>> {
  try {
    const session = await requireEdit("stp");
    const staffId = session.id;
    ...
    const needsWithholding = isWithholdingTarget(agent);
    const withholdingTaxAmount = needsWithholding
      ? calcWithholdingTax(input.amount)
      : null;
    const netPaymentAmount =
      withholdingTaxAmount != null ? input.amount - withholdingTaxAmount : null;
    ...
    const note =
      input.note?.trim() ||
      `${input.agentName} ${expenseTypeLabels[input.expenseType] || input.expenseType}${
    ...
    const transaction = await prisma.transaction.create({
      data: {
        ...
        note,
        isWithholdingTarget: needsWithholding,
        withholdingTaxRate: needsWithholding ? 10.21 : null,
    ...
    console.error("[createConfirmedTransactionFromExpense] error:", e);
    return err(e instanceof Error ? e.message : "予期しないエラーが発生しました");
  }
}
