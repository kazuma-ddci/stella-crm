...
import type { PaymentGroupMailFormData } from "./mail-actions";
import {
...
export function PaymentGroupMailModal({
  open,
  onClose,
  paymentGroupId,
}: Props) {
  ...
  const buildTemplateVariables = useCallback(
    (formData: PaymentGroupMailFormData): Record<string, string> => {
      const group = formData.paymentGroup;
      const now = new Date();
      return {
  ...
  const applyTemplate = useCallback(
    (
      template: PaymentGroupMailFormData["templates"][number],
      formData?: PaymentGroupMailFormData
    ) => {
  ...
  const addContactAsRecipient = useCallback(
    (
      contact: PaymentGroupMailFormData["contacts"][number],
      type: "to" | "cc" | "bcc"
    ) => {
  ...
  const handleSend = useCallback(async () => {
    ...
    setSending(true);
    try {
      const result = await sendPaymentGroupMail({
        paymentGroupId,
      ...
      if (result.success) {
        ...
        if (data?.paymentGroup.stellaCompanyId && manualRecipients.length > 0) {
          setPendingContacts(
  ...
  const handleResend = useCallback(
    async (mailId: number) => {
      ...
      setSending(true);
      try {
        const result = await resendPaymentGroupMail(mailId);
        if (result.success) {
  ...
  const handleAddContacts = useCallback(async () => {
    if (!data?.paymentGroup.stellaCompanyId || pendingContacts.length === 0) return;
    setSending(true);
    try {
      for (const contact of pendingContacts) {
        await addContact(data.paymentGroup.stellaCompanyId, {
          name: contact.name,
          email: contact.email,
          isPrimary: false,
        });
