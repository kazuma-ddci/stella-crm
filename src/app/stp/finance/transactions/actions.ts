...
import type { TransactionFormData } from "@/app/accounting/transactions/actions";
import {
  getSystemProjectContext,
  isTemplateContainingProject,
...
export async function listTransactions(
  ...
  return records.map((r) => ({
    ...
    scheduledPaymentDate: r.scheduledPaymentDate
      ? r.scheduledPaymentDate.toISOString().split("T")[0]
      : null,
    status: r.status,
    note: r.note,
...
export async function getStpTransactionFormData(): Promise<TransactionFormData> {
  const [formData, ctx] = await Promise.all([
    getTransactionFormData(),
  ...
  return {
    ...formData,
    costCenters: formData.costCenters.filter((cc) =>
      cc.projectIds.includes(ctx.projectId)
    ),
    allocationTemplates: formData.allocationTemplates.filter((t) =>
      isTemplateContainingProject(t, ctx.costCenterIds)
    ),
  };
}
...
export async function stpConfirmTransaction(id: number) {
  await validateTransactionScope(id, "stp");
  return confirmTransaction(id);
}

export async function stpUnconfirmTransaction(id: number, reason: string) {
  await validateTransactionScope(id, "stp");
  return unconfirmTransaction(id, reason);
}

export async function stpResubmitTransaction(id: number, body?: string) {
  await validateTransactionScope(id, "stp");
  return resubmitTransaction(id, body);
}

export async function stpSubmitToAccountingTransaction(id: number) {
  await validateTransactionScope(id, "stp");
  return submitToAccountingTransaction(id);
}
