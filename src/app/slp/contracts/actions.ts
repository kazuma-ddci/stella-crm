 ...
 * 自動同期のON/OFF切替
 */
export async function toggleAutoSync(contractId: number) {
  ...
  if (newAutoSync && contract.cloudsignDocumentId) {
    try {
      ...
      console.error("[SLP Contract] Auto-sync re-enable sync failed:", err);
    }
  }
 ...
 */
export async function manualSync(contractId: number) {
  await requireProjectMasterDataEditPermission("slp");
  ...
  if (!contract) throw new Error("契約書が見つかりません");
  if (!contract.cloudsignDocumentId) throw new Error("CloudSign書類IDがありません");
  ...
  const clientId = project?.operatingCompany?.cloudsignClientId;
  if (!clientId) throw new Error("CloudSign APIキーが設定されていません");
 ...
 * リマインド送付
 */
export async function remindContract(contractId: number) {
  await requireProjectMasterDataEditPermission("slp");
 ...
 * ステータスを手動変更
 */
export async function updateContractStatus(contractId: number, newStatusId: number, note?: string) {
  await requireProjectMasterDataEditPermission("slp");
  ...
  if (!newStatus) throw new Error("ステータスが見つかりません");
