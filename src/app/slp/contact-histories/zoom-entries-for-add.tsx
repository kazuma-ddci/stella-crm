...
import { listZoomLinkedStaffs } from "./zoom-actions";

export type ZoomAddEntry = {
  zoomUrl: string;
  hostStaffId: string; // string for select value compat
...
/**
 * 接触履歴「新規追加」フォーム内に配置するZoom議事録連携エントリエディタ
 * - 初期は何も表示されない（Pattern B）
 * - 「+ Zoom議事録連携を追加」ボタンで1つずつ展開
 * - 各エントリは × ボタンで削除可能
 * - 親コンポーネント側が entries state を保持し、保存時に処理する
 */
export function ZoomEntriesForAdd({
  ...
  // entries が空でなくなった時だけスタッフ一覧取得（パフォーマンス）
  // 非同期完了コールバック内での setState のみなので set-state-in-effect にはならない
  useEffect(() => {
    if (entries.length === 0) return;
    if (staffsLoaded) return;
    let cancelled = false;
    listZoomLinkedStaffs().then((r) => {
      if (cancelled) return;
      if (r.ok) {
