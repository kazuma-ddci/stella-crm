...
const OUT_DIR = path.join(__dirname, "..", "tmp", `cw-modal-${PHASE}`);
...
async function main() {
  ensureDir(OUT_DIR);
  console.log(`\n=== Contact History Modal Screenshots (${PHASE}) ===`);
  ...
  try {
    ...
    // 1. Contact History Modal - Companies
    console.log("[1/2] Contact History Modal (companies)");
    ...
    // 2. Contact History Modal - Agents
    console.log("[2/2] Contact History Modal (agents)");
    await captureModalAtViewports(
      page,
