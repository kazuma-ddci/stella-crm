...
[ユーザー] 予約カレンダーURLを開く
   ...
   - form3-5: token（hidden）
   - form3-6,7: 業種/職種・上場/未上場（ユーザーが手動入力）
   ↓ 「予約する」ボタン押下
[プロライン] → 予約webhook
[CRM webhook]
...
GET /api/public/slp/briefing-reservation
  ...
  &form3-6=[[form3-6]]   # 業種/職種
  &form3-7=[[form3-7]]   # 上場/未上場
  &secret=...
```
...
| form3-6 | 業種/職種 | 選択項目 | - | 選択肢に「その他」を必ず入れる。事前送信では「その他」を入れる。CRM側の業種/職種マスタに未登録なら自動追加し、企業にも自動紐付け |
| form3-7 | 上場/未上場 | 選択項目 | - | 選択肢に「上場」「未上場」を入れる。事前送信では「未上場」を入れ、想定外文字列は「不明」として扱う |
...
- `industryJobFormAnswer String?` — 概要案内フォームの業種/職種回答
- `listingStatus String?` — 上場区分（`listed` / `unlisted` / `unknown`）
