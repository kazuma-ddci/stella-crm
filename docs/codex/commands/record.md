...
- ガードテスト: `src/__tests__/specs/SPEC-XXX-NNN.test.ts`
- 手動確認手順: [手順があれば]
...
確定仕様をコードで保護するテストを `src/__tests__/specs/SPEC-XXX-NNN.test.ts` に作成する。

**作成ルール：**

- テスト可能な関数がある場合（表示フォーマット、計算ロジック等）→ ガードテストを作成
- UI表示パターンのみでテスト不可の場合 → スキップし、SPECドキュメントの検証方法に「手動確認のみ」と明記
 ...
 * SPEC-XXX-NNN: [タイトル]
 *
 * @see docs/specs/SPEC-XXX-NNN.md
 *
 * このテストは確定仕様のガードテストです。
...
describe("SPEC-XXX-NNN: [タイトル]", () => {
  // 正常系: 仕様通りの出力を検証
  it("[仕様に基づく期待動作]", () => {
    // ...
  });

  // 禁止事項の検証: forbidden_changes に対応
  describe("禁止事項の検証", () => {
    it("❌ [禁止事項]", () => {
      // ...
    });
...
**重要:**
- SPECドキュメントの `禁止事項（forbidden_changes）` を全てテストケースに含める
- テスト対象の関数が存在しない場合は、`src/lib/format/` や `src/lib/` に純粋関数として切り出してからテストを書く
- 既存テストファイルがある場合は上書きではなく追記する
...
ガードテスト: src/__tests__/specs/SPEC-STP-002.test.ts（作成 / スキップ）
```
