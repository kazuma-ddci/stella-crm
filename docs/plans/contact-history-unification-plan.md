...
## 【2026-04-24 追記】フォーム仕様と外部連携ビジョン（要約）

塩澤さんとのすり合わせで確定したフォーム UI 仕様と、外部アプリとの役割分担を記録。

### フォーム仕様の確定

| 項目 | 旧仕様 | 新仕様 |
|---|---|---|
| 弊社スタッフ > ホスト | ドロップダウンあり | **削除**。`createdByStaffId` (作成者) のみ裏で記録 |
| 会議 > 状態 (state) | ユーザーが選択 | **読み取り専用表示のみ**。API/webhook で自動遷移 |
| 会議 > ラベル | 常に1行目に表示 | **会議セクションの最下段**に配置。通常は目立たせない |
| 会議 > 参加URL (joinUrl) | `startUrl` と別欄 | **「URL」の1欄に統合** |
| 会議 > ホスト用URL (startUrl) | 入力欄あり | **フォームから削除**。API自動生成時のみ裏で保存 |
| 会議 > パスコード | 常時表示 | 詳細設定へ折りたたみ（MVP では非表示でOK） |
| 会議 > 予定日時 | 個別日時入力 | **削除**。基本情報の日時を継承 |
| 会議 > ホスト | ドロップダウンあり | **残す**。参加スタッフから選択（API連携の基準） |
| 基本情報 > status と 会議 > state | 独立 | **連動**。status 変更で会議 state が自動遷移 |

### status × state の連動ルール

```
基本情報.status = "scheduled" (予定)
  → すべての会議 state = "予定"
  → urlSource により分岐:
      - auto_generated: Zoom/Meet API 即時発行
      - manual_entry: 手入力 URL をそのまま保存
      - empty: URL なし
  → Googleカレンダー連携が設定済なら、参加スタッフ全員のカレンダーに予定作成+URL挿入

基本情報.status = "completed" (実施済)
  → 各会議:
      joinUrl + ホストAPI連携済 + externalMeetingId 抽出可 → state="取得中" → API取得 → "完了" or "失敗"
      それ以外 → state="完了" (API呼ばず、マークのみ)
  → AI要約取得できたら meetingMinutes に自動追記 (minutesAppendedAt で二重追記防止)

基本情報.status = "cancelled" (キャンセル)
  → API呼び出しスキップ
  → Googleカレンダーから削除

基本情報.status = "rescheduled" (リスケ)
  → 会議 state は "予定" のまま、日時のみ更新
  → Googleカレンダーの日時も更新
```

### 複数会議の用途 (確認済)

- **想定ユースケース**: Zoom が誤って切断された / 制限時間切れで再リンク発行
- **想定外**: 別時刻での会議は「別の接触履歴」として作成する
- API 取得ロジックで特別扱いしないなら複数会議の UI 上の意義は薄い
  → MVPでは **1接触1会議** を前提に UI を設計し、複数追加は「もう1つ追加」ボタン経由の例外扱いで良い

### 外部アプリ (Slack/Telegram ミドルウェア) との役割分担

**ビジョン**:
- Slack/Telegram → **別アプリ (ミドルウェア)** を経由して CRM を操作
- ミドルウェアは **CRM と Google カレンダー両方を読み取り/書き込み**
- ミドルウェアの責務:
  - 接触履歴の新規追加・編集・削除 (Slack/Telegram コマンドから)
  - スタッフの追加・削除・URL変更・時間変更
  - **個人 vs 全体の変更判定ロジック** (CRM 側では素朴に同期しない)
    - 例: スタッフAがカレンダーで自分の予定だけ時間変更 → ミドルウェアが検知 → 「全員変更しますか? あなただけですか?」と Slack で確認
    - 例: スタッフBが自分の予定を削除 → 「会議自体を削除ですか? あなただけ欠席ですか?」
  - Slack/Telegram は**ミドルウェアの UI**

**CRM 側 (このアプリ) の責務**:
- 接触履歴の Source of Truth
- Googleカレンダー **初期作成** (予定時点で参加スタッフ全員のカレンダーに一括作成)
- 会議URL の取得・保存 (API 連携)
- 録画・議事録・AI要約の取り込み
- REST API をミドルウェアに公開 (接触履歴 CRUD / 会議情報 / 参加者)

**CRM 側で実装しないこと**:
- 個人カレンダー変更の Webhook 受信 (ミドルウェアが受ける)
- 個別 vs 全体の変更判定ロジック (ミドルウェアが判定)
- Slack/Telegram ボット実装 (ミドルウェアの管轄)

### Googleカレンダー 初期作成のイメージ

- 接触履歴作成 (status=予定) 時:
  - 参加スタッフ全員の Google カレンダーに予定作成
  - イベントのメモ欄に会議 URL を挿入
  - 時間になったら各自のカレンダーを開いて URL をクリックすればすぐ入室

### 残メモ (実装時参照)

- `startUrl` フィールド: DB からは削除しない (API自動生成時に使う)。フォームからだけ削除。
- `passcode`: 同上、内部保持、フォーム簡略化
- `state`: DB 値は維持、フォームで表示のみ
- 弊社スタッフのホスト (`contact_history_meetings.hostStaffId` 以外の「全体ホスト」概念): **DBに別カラムがないので何も削除不要**。フォームの UI フィールドだけ除去。
- 会議の `scheduledStartAt/EndAt`: DB カラムは残す (将来の拡張余地)、フォームから編集UIは撤去
